...
Supporting data: operator position from device GPS; ground elevation at the launch point via a resilient chain — cached last-good value per launch area, then Open-Meteo (direct, then via worker), then USGS EPQS (`epqs.nationalmap.gov`, direct then via worker) — used to convert AMSL floors to AGL; local airport database embedded in the app, sourced from FAA Part 71 designations.
...
A rendering reference reproducing every verdict this ladder can produce — card states, TFR status-line states, and scope symbology — is maintained in the repository as `airspace_reference.html`, for crew training and reviewer orientation. It must be updated alongside this document when renderings change.
