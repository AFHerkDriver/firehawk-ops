...
- **v2.6** — **Pilot Training** (`training.html`): the Appendix E onboarding checklist goes electronic — shared app sign-in opens each crew member's own training folder, with instructor-only sign-offs (owner-set Instructor toggle in crew management), crew-linked trainee records, owner-gated PM approval, and an **In Training** roster chip that clears on PM sign-off (new pilots start In Training by default). **My Shifts** auto-hides past dates and flags planned out-of-service windows on assigned aircraft; command board adds a day-level UPSTAFF section and an on-duty RPIC contact button (current release).
